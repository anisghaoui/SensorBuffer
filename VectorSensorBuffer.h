#pragma once
#include "BaseSensorBuffer.h"
#include <iostream>
#include <vector>


template <typename T>
class VectorSensorBuffer : public BaseSensorBuffer<T>
{	
	std::vector< SensorData<T> > buffer;		//Warning prior to C++11 putting < Type <type >> was considered a sentax error as >> is a stream opertor

	void store(SensorData<T>& data);
	void retreive(int sensorId);
	void saveBufferToFile(const char* outName);
};



template <typename T>
void VectorSensorBuffer<T>::store(SensorData<T>& data)
{
	buffer.push_back(data);
}


template <typename T>
void VectorSensorBuffer<T>::retreive(int sensorId)
{
	for (auto it = buffer.begin(); it != buffer.end() ;it++)
	{
		if( (*it).get_id() ==  sensorId ){
			buffer.erase(it);
			break;
		}
	}
}

template <typename T>
void VectorSensorBuffer<T>::saveBufferToFile(const char* outName)
{
	ofstream outFile;
	outFile.open(outName);
	if (!outFile)
	{
		cerr << "Unable to create file named "<<outName<<"; maybe you don't have the right permissions ?"<<endl;
		exit(2);
	}
	for (auto i = buffer.begin(); i != buffer.end(); ++i)
	{
		outFile << (*i).get_id() << "  " << (*i).get_value()<<endl;	
	}
	outFile.close();
}
